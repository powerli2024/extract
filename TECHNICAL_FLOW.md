...
无论输入文件是否已经符合赛题格式，送入 ClearVoice/ONNX 前统一执行：

```text
多声道 (T,C)/(C,T) ──► 通道平均 ──► 重采样 16 kHz
                  ──► 振幅裁剪 [-1,1] ──► 峰值归一化到 0.70
```

默认赛题音频是单声道 16 kHz，因此该步骤通常只会做幅值检查和归一化；
外部音频或诊断脚本则不会绕过同一契约。实现位于
`scripts/utils_audio.py::preprocess_for_separation`，返回值始终标记为
`sr=16000`。空音频和非有限值直接失败，不进入模型。
